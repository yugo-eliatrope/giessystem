# 🌱 GiesSystem

Система мониторинга температуры и влажности, а также управление поливом на базе Arduino с веб-интерфейсом.

## Возможности

- 🌡️ Мониторинг температуры и влажности (DHT11)
- 💧 Управление насосом через реле
- 📊 Веб-интерфейс для отображения данных
- 📋 Логирование системных событий

## Схема подключения Arduino

```
Arduino Uno/Nano
├── Pin 2  → DHT11 (Data)
├── Pin 7  → Relay Module (IN)
├── 5V     → DHT11 (VCC), Relay (VCC)
└── GND    → DHT11 (GND), Relay (GND)
```

## Установка

### 1. Arduino

1. Установите библиотеку `DHT sensor library` через Arduino IDE
2. Откройте `arduino/app.ino`
3. Загрузите скетч на плату

### 2. Сервер

```bash
# Установка зависимостей
yarn install

# Сборка
yarn build
```

### 3. Конфигурация

Создайте файл `.env` в корне проекта:

```env
DATABASE_URL="file:./dev.db"
SERIAL_PORT_PATH=/dev/ttyUSB0
SERIAL_BAUD_RATE=9600
SERVER_PORT=3000
MAX_LOG_MESSAGES=100
```

> **Примечание:** Путь к Serial порту зависит от ОС:
> - Linux: `/dev/ttyUSB0` или `/dev/ttyACM0`
> - macOS: `/dev/cu.usbserial-*` или `/dev/cu.usbmodem*`
> - Windows: `COM3`, `COM4`, и т.д.

### 4. База данных

Проект использует SQLite через Prisma. Для инициализации БД:

```bash
# Создать/обновить базу данных по схеме
yarn db:push
```

Другие полезные команды:

```bash
# Просмотр данных в браузере (Prisma Studio)
yarn db:studio

# Создание миграции (для production)
yarn db:migrate
```

### 5. Запуск

```bash
# Production
yarn start

# Development
yarn dev
```

Веб-интерфейс будет доступен по адресу `http://localhost:3000` (если `SERVER_PORT=3000`)

## API

| Endpoint | Метод | Описание |
|----------|-------|----------|
| `/` | GET | Веб-интерфейс |
| `/info` | GET | JSON с данными температуры, влажности и логами |
| `/pump?time=N` | GET | Включить насос на N секунд (2-32) |

## Удалённый доступ через SSH туннель

Если сервер запущен на домашнем компьютере, можно пробросить порт через VPS для доступа извне.

### 1. Настройка VPS

Убедитесь, что SSH разрешает проброс портов:

```bash
sudo nano /etc/ssh/sshd_config
```

Проверьте/добавьте:

```
AllowTcpForwarding yes
GatewayPorts yes
```

Перезапустите SSH:

```bash
sudo systemctl restart sshd
```

### 2. Установка autossh

На домашнем компьютере:

```bash
# Debian/Ubuntu
sudo apt install autossh

# macOS
brew install autossh
```

### 3. Запуск туннеля

```bash
autossh -M 0 -N -R 0.0.0.0:3000:localhost:3000 user@VPS_IP
```

Параметры:
- `-M 0` — отключить мониторинг порта (используется ServerAliveInterval)
- `-N` — не выполнять команды, только проброс
- `-R 0.0.0.0:3000:localhost:3000` — проброс порта 3000 VPS на локальный 3000

### 4. Автозапуск (systemd)

Создайте файл `/etc/systemd/system/giessystem-tunnel.service`:

```ini
[Unit]
Description=GiesSystem SSH Tunnel
After=network.target

[Service]
User=YOUR_USER
ExecStart=/usr/bin/autossh -M 0 -N -o "ServerAliveInterval 30" -o "ServerAliveCountMax 3" -R 0.0.0.0:3000:localhost:3000 user@VPS_IP
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
```

Активируйте:

```bash
sudo systemctl enable giessystem-tunnel
sudo systemctl start giessystem-tunnel
```

## Структура проекта

```
giessystem/
├── arduino/
│   └── app.ino          # Скетч для Arduino
├── src/
│   ├── index.ts         # Точка входа
│   ├── config.ts        # Конфигурация из .env
│   ├── server.ts        # HTTP сервер
│   ├── serial-manager.ts # Работа с Serial портом
│   ├── parser.ts        # Парсинг данных от Arduino
│   ├── store.ts         # Хранение состояния
│   └── logger.ts        # Логирование
├── public/
│   └── index.html       # Веб-интерфейс
├── dist/                # Скомпилированные JS файлы
└── package.json
```

## Лицензия

ISC

